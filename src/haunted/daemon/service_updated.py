...
class HauntedDaemonUpdated:
    ...
    async def start(self):
        ...
        try:
            ...
            # Start worker pool
            max_workers = min(
                self.config.ai.max_concurrent_issues, 3
            )  # Limit for stability
        ...
        while self.running:
            try:
                ...
                for issue_dict in issues:
                    ...
                    logger.info(
                        f"Issue {issue_id}: status='{status}', stage='{stage}', active={issue_id in self.active_issues}"
                    )
                ...
                logger.info(
                    f"Scanner sleeping for {self.config.daemon.scan_interval} seconds"
                )
                await asyncio.sleep(self.config.daemon.scan_interval)
            ...
            try:
                ...
                try:
                    issue_dict = await asyncio.wait_for(
                        self.issue_queue.get(), timeout=5.0
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Worker {worker_id} failed processing issue {issue_id}: {e}"
                    )
        ...
        try:
            ...
            if result["workflow_stage"] == "done":
                if result["status"] == "closed":
                    logger.info(
                        f"[{worker_id}] ✅ Issue {issue_id} completed successfully"
                    )
                    ...
                    logger.warning(
                        f"[{worker_id}] ⚠️ Issue {issue_id} finished but blocked: {result.get('diagnosis_log', 'Unknown issue')}"
                    )
            else:
                logger.warning(
                    f"[{worker_id}] ⚠️ Issue {issue_id} did not complete workflow"
                )
            ...
            try:
                diagnosis = await self.workflow_engine.diagnose_issue(
                    issue_dict, str(e)
                )
                logger.info(
                    f"[{worker_id}] Diagnosis for {issue_id}: {diagnosis[:200]}..."
                )
            except Exception as diag_error:
                logger.warning(
                    f"[{worker_id}] Could not diagnose issue {issue_id}: {diag_error}"
                )
    ...
    async def _handle_git_operations(self, result: Dict[str, Any], worker_id: str):
        ...
        try:
            ...
            commit_msg = self.config.git.commit_message_template.format(
                issue_id=issue_id, stage="completed", description=result["title"]
            )

            logger.info(
                f"[{worker_id}] Git operations for {issue_id} would commit: {commit_msg}"
            )
    ...
    def _log_final_stats(self):
        ...
        success_rate = (
            (self.processed_issues / (self.processed_issues + self.failed_issues) * 100)
            if (self.processed_issues + self.failed_issues) > 0
            else 0
        )

    ...
    async def _scan_issues(self):
        ...
        while self.running:
            try:
                ...
                new_issues = [
                    issue for issue in issues if issue.id not in self.active_issues
                ]
    ...
    async def _process_issue(self, issue: Issue, worker_id: str):
        ...
        try:
            ...
            try:
                ...
                # Add error comment
                await self.db_manager.add_comment(
                    issue.id, "ai", f"Issue processing failed: {str(e)}"
                )
            except Exception as update_error:
                logger.error(
                    f"Failed to update blocked issue {issue.id}: {update_error}"
                )

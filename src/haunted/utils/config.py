...
class AIConfig(BaseModel):
    ...
    model: str = Field(
        default="claude-3-sonnet-20240229", description="Model reference"
    )
    max_tokens: int = Field(default=4000, description="Maximum tokens for responses")
    max_concurrent_issues: int = Field(
        default=3, description="Max concurrent issues to process"
    )
...
class GitConfig(BaseModel):
    ...
    commit_message_template: str = Field(
        default="Issue #{issue_id}: {stage} - {description}"
    )
...
class ConfigManager:
    ...
    def create_default_config(self) -> HauntedConfig:
        ...
        """
        config = HauntedConfig(project_root=str(self.project_root))

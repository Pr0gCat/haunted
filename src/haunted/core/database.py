...
class DatabaseManager:
    ...
    async def create_phase(self, name: str, description: Optional[str] = None) -> dict:
        ...
        phase = Phase(name=name, description=description, branch_name=branch_name)
    ...
    async def list_phases(self) -> List[dict]:
        ...
        async with self.get_session() as session:
            ...
            result = await session.execute(
                text(
                    "SELECT id, name, description, status, branch_name, created_at, updated_at FROM phases"
                )
            )
            phase_list = []
            for row in result:
    ...
    async def get_tasks(self, issue_id: int) -> List[Task]:
    ...
    # Comment operations
    async def add_comment(self, issue_id: int, author: str, content: str) -> Comment:
        """Add comment to issue."""
        comment = Comment(issue_id=issue_id, author=author, content=content)
    ...
    async def get_comments(self, issue_id: int) -> List[Comment]:
    ...
    async def get_test_results(
        self, issue_id: int, test_type: Optional[TestType] = None
    ) -> List[TestResult]:
        """
